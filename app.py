...
with st.sidebar:
    ...
    char_img_file = char_data.get("image_file")
    char_img_path = ASSETS_DIR / char_img_file
    
    if char_img_path.exists():
        st.image(str(char_img_path), use_column_width=True)
    else:
        st.warning(f"⚠️ Missing local file: Cannot find `{char_img_file}` inside `{ASSETS_DIR}`")
        st.image("https://via.placeholder.com/400x400.png?text=Missing+Character+Image", use_column_width=True)

    # ── LOCATION IMAGE LOADER ──
    st.markdown("**Setting:**")
    loc_img_file = loc_data.get("image_file")
    loc_img_path = ASSETS_DIR / loc_img_file
    
    if loc_img_path.exists():
        st.image(str(loc_img_path), use_column_width=True)
    else:
